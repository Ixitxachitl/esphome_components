...
static const char *TAG = "mfrc522_i2c";
...
std::string MFRC522I2C::get_uid() {
  ...
  if (this->request_tag(rc522::RC522::PiccCommand::PICC_CMD_REQA) && this->anti_collision(buffer, &buffer_size)) {
...
std::string MFRC522I2C::get_fifo_data_string() {
  uint8_t fifo_size = this->pcd_read_register(rc522::RC522::PcdRegister::FIFO_LEVEL_REG);
  uint8_t buffer[64];
  this->pcd_read_register(rc522::RC522::PcdRegister::FIFO_DATA_REG, fifo_size, buffer, 0);
  this->fifo_data_ = std::string(buffer, buffer + fifo_size);
  return this->fifo_data_;
}

bool MFRC522I2C::request_tag(rc522::RC522::PiccCommand command) {
  this->pcd_write_register(rc522::RC522::PcdRegister::BIT_FRAMING_REG, 0x07);
  uint8_t command_buffer[] = {static_cast<uint8_t>(command)};
  this->pcd_transceive_data_(command_buffer[0]);
  return true;
}

bool MFRC522I2C::anti_collision(uint8_t *buffer, uint8_t *buffer_size) {
  this->pcd_write_register(rc522::RC522::PcdRegister::BIT_FRAMING_REG, 0x00);
  this->pcd_read_register(rc522::RC522::PcdRegister::FIFO_DATA_REG, *buffer_size, buffer, 0);
  return true;
}
